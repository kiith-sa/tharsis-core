...
/// 
/// Used when executing process Process to read past entities and their components 
/// of types specified by InComponents, and writing future components for future
/// versions of these entities.
///
/// Can be passed to a process() method of a process, providing direct access to
/// components of any past entity and handle of the current entity.
...
private:
    // True if the Process does not write to any future component. Usually
    // processes that only read past components and produce some kind of output.
    ...
    /// Index of the first future component (in the future component buffer) for
    /// the current entity.
    ///
    ...
    /// (CTFE) Get the name of the component buffer data member for specified
    /// component type.
    ...
    /// (CTFE) For each processed (past) component type, generate 2 data 
    /// members: component buffer and component count buffer.
    ...
    /// Mixin typed slices to access past components, and pointers to component
    /// count buffers to read the number of components of a type per entity.
    ///
...
package:
    ...
    /// should be written (although process() may still decide not to write it).
    ...
    /// Get a slice to where to write future (multi) components for the current
    /// entity. The slice is at least FutureComponent.maxComponentsPerEntity
    /// long (the process() method may shorten it after passed).
    ...
    ///
    /// May be called more than once while processing an entity, but the final
    /// number must match the number of components actually written.
    ///
    ...
    bool matchComponents(ComponentTypeIDs...)() @trusted
    {
        ...
        static string matchCode()
        {
            ...
            // result of multiplying them all is 0. Af all are at least 1, the
            // result is true.
            string[] parts;
...
private:
    ...
    void nextFutureEntity() @safe pure nothrow
    {
        static if(!noFuture)
        {
            ...
            futureComponents_.offsets[futureEntityIndex_] 
                = futureComponentOffset_;
