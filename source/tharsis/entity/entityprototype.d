...
private:
     ...
     * sorted) components_ (not in reverse order), and moved to the end of storage_. It
     * may start right after components_ or after an alignment gap.
     *
     * For MultiComponents, one ID is stored per component even if they have the same type.
     */
...
public:
     ...
     *
     * Used to determine minimum memory to pass to EntityPrototype.useMemory(). Most
     * prototypes are likely to be very small; this is the size of memory needed to avoid
     * *any* prototype running out of memory.
     *
        ...
        @trusted nothrow
    {
        ...
        assert(storage_.length % 16 == 0, "EntityPrototype storage length not aligned to 16");
 ...
 *         memory         = Memory to use for the new entity prototype. Must be at least
     ...
     ubyte[] memory, const(ComponentTypeInfo)[] componentTypes) nothrow
{
    ...
    {
        // baseRange, overRange are sorted by component type. If baseRange.front.typeID is
        // less, there's no component with that ID in overRange so use component/s from
        // baseRange. Same for overRange.front.typeID and overRange. If the type IDs are
        // equal, both baseRange and overRange have this component and over overrides base.
        // If either is empty, we use the other one.
        ...
        // Copies to result components of the current type. Skips components of this type
        // in the ignored buffer. (Which is used, which ignored depends on whether there's
        // a component of this type in base and over - over overrides base.)
             ...
             ref EntityPrototype.ConstComponentRange ignoredRange) nothrow
        {
            ...
            // Copy all components until we reach components of a different type or until
            // usedIDs is empty.
